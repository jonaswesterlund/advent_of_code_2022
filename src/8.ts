...
function countVisibleTrees(trees: number[][]) {
  ...
  for (let row = 0; row < trees.length; row++) {
    for (let col = 0; col < trees.length; col++) {
      ...
      const higherLeft = trees[row].filter(
        (tree, index) => index < col && tree >= selectedTree
      );
      const higherRight = trees[row].filter(
        (tree, index) => index > col && tree >= selectedTree
      );
...
function calculateViewingDistance(trees: number[][]) {
  let scenicScores = new Array(trees.length).fill(
    new Array(trees.length).fill(1)
  );
  for (let row = 0; row < trees.length; row++) {
    for (let col = 0; col < trees.length; col++) {
      if (
      ...
      const treesLeft = trees[row]
        ...
        .reverse();
      const firstIndexLeft = treesLeft.findIndex(
        (tree) => tree >= selectedTree
      );
      const treesRight = trees[row].filter((tree, index) => index > col);
      const firstIndexRight = treesRight.findIndex(
        (tree) => tree >= selectedTree
      );
      const treesUp = trees
        .map((row) => row[col])
      ...
      const treesDown = trees
        .map((row) => row[col])
        .filter((tree, index) => index > row);
      const firstIndexDown = treesDown.findIndex(
        (tree) => tree >= selectedTree
      );
      const indexes = [
        firstIndexLeft,
