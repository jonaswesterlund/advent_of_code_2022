...
const input = readFile("3_rucksack");
let prioritySum = 0;
for (let rucksack of input.split("\n")) {
    ...
    const priority = charAsciiDecimal >= 97 ? charAsciiDecimal - 96 : charAsciiDecimal - 38;
...
for (let i = 0; i < inputArray.length; i += 3) {
    ...
    const commonItems = Array.from(elf1).filter((c) => Array.from(elf2).includes(c) && Array.from(elf3).includes(c));
    if (commonItems.length === 0) {
        throw Error("this is a problem");
    }
    const commonItem = commonItems[0];
